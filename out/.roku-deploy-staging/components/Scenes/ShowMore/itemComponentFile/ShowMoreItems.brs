sub Init()
    ...
    if getThumbnailOrientaion() = "LANDSCAPE"
        ...
        if m.top.itemContent.itemType = "BANNER"
            m.showMoreDetailPoster.uri = m.top.itemContent.HDPosterURLPortrait
            m.showMoreDetailPoster.failedBitmapUri = getPLACEHOLDER_IMAGE_PORTRAIT ()
            m.showMoreDetailPoster.loadingBitmapUri = getPLACEHOLDER_IMAGE_PORTRAIT ()
        end if
    end if
...
sub OnContentSet()
    ...
    m.showMoreDetailPoster.loadDisplayMode = "noScale"
    if m.top.height < 400 and m.top.width < 400
        m.showMoreDetailPoster.loadWidth = 400
        m.showMoreDetailPoster.loadHeight = 500
    end if

    updateLayout()
    if m.top.itemContent <> invalid
        orientation = m.top.itemContent.thumbnail_orientation
        if orientation = invalid or orientation = ""
            orientation = getThumbnailOrientaion()
        end if

        if orientation <> invalid and orientation <> ""
            if orientation = "LANDSCAPE"
                m.showMoreDetailPoster.failedBitmapUri = getPLACEHOLDER_IMAGE()
                m.showMoreDetailPoster.loadingBitmapUri = getPLACEHOLDER_IMAGE()
                m.showMoreDetailPoster.uri = m.top.itemContent.HDPOSTERURL
