...
' Component initialization, setting default properties, uring observers (handlers)
sub init()
...
sub On_signupAuthFlow_isAuthorized()
    ? "On_signupAuthFlow_isAuthorized"
...
? "********************** inside On_isAfterLoginAuthFlow "
    if GetParentScene() = invalid then
        return
    end if
